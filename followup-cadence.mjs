#!/usr/bin/env node
/**
 * followup-cadence.mjs — Follow-up Cadence Tracker for learn-ops
 *
 * Parses enrollments.md + follow-ups.md, calculates follow-up cadence
...
const APPS_FILE = existsSync(join(LEARN_OPS, 'data/enrollments.md'))
  ? join(LEARN_OPS, 'data/enrollments.md')
  : join(LEARN_OPS, 'enrollments.md');
const FOLLOWUPS_FILE = join(LEARN_OPS, 'data/follow-ups.md');
const PROFILE_FILE = process.env.LEARN_OPS_PROFILE || join(LEARN_OPS, 'config/learner-profile.yml');
...
// --- Resolve report path ---
export function resolveReportPath(reportField, appsFile = APPS_FILE, repoRoot = LEARN_OPS) {
  ...
  // `../reports/...` (the data/enrollments.md layout) escape above the project.
