...
/**
 * doctor.mjs — Setup validation for learn-ops
...
// --strict adds a live ATS-slug probe of learning-portals.yml (network). Opt-in so the
...
function checkPlaywrightMcp(root) {
  ...
  return {
    ...
    fix: [
      ...
      'Tracking: https://github.com/santifer/learn-ops/issues/506',
    ],
  };
}
...
const USER_LAYER_PREREQS = [
  {
    ...
    fix: [
      'Create learner-profile.md in the project root with your IT background + transition target',
      'Use the template sections in the file (Current IT Background / Transition Target / Artifacts / Constraints)',
    ],
  },
  {
    path: 'config/learner-profile.yml',
    fix: [
      'Run: cp config/learner-profile.example.yml config/learner-profile.yml',
      'Then edit it with your target_track, archetype_id, time budget, and learning_goals',
    ],
  },
  {
    ...
    fix: [
      ...
      'Then map your background to your chosen archetype',
    ],
  },
  {
    path: 'learning-portals.yml',
    fix: [
      'Run: cp templates/learning-portals.example.yml learning-portals.yml',
      'Then customize with the free-training sources you want to scan',
    ],
  },
];
...
// --strict only: probe the ATS slug of every tracked company in learning-portals.yml so
// a typo'd slug (which 404s silently on scans) surfaces here. Skipped gracefully
// when learning-portals.yml is absent. Delegates to verify-portals.mjs so there is one
// slug-probing implementation. Network-bound, hence opt-in.
async function checkPortalSlugs(root) {
  const portalsPath = join(root, 'learning-portals.yml');
  if (!existsSync(portalsPath)) {
    return { pass: true, label: 'ATS slugs: no learning-portals.yml yet (skipped)' };
  }
  try {
    ...
    if (unresolved.length === 0) {
      return { pass: true, label: 'All ATS slugs in learning-portals.yml resolve' };
    }
    return {
      pass: false,
      label: `${unresolved.length} ATS slug(s) in learning-portals.yml do not resolve`,
      fix: [
...
Paste job URLs below as \`- [ ] {url}\` then run \`/learn-ops pipeline\`.
...
function checkPipelineFile() {
  const filePath = join(projectRoot, 'data', 'learning-inbox.md');
  if (existsSync(filePath)) {
    return { pass: true, label: 'data/learning-inbox.md ready' };
  }
  try {
    writeFileSync(filePath, PIPELINE_SKELETON, 'utf-8');
    return { pass: true, label: 'data/learning-inbox.md ready (auto-created)' };
  } catch {
    return {
      pass: false,
      label: 'data/learning-inbox.md could not be created',
      fix: 'Run: mkdir -p data && touch data/learning-inbox.md',
    };
  }
}
