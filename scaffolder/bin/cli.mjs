#!/usr/bin/env node
// learn-ops scaffolder — one-command install.
...
const REPO = "https://github.com/santifer/learn-ops.git";
const LATEST_RELEASE = "https://api.github.com/repos/santifer/learn-ops/releases/latest";
const NPM = process.platform === "win32" ? "npm.cmd" : "npm";

// learn-ops is AI-agnostic: every one of these CLIs reads AGENTS.md and works
...
const USAGE = `learn-ops — set up an AI job search workspace.

Usage:
  npx learn-ops init [folder]    Create a new workspace (default: ./learn-ops)

After setup, open your AI coding tool inside the folder and paste a job offer.
Docs: https://github.com/santifer/learn-ops`;
...
async function latestTag() {
  try {
    const res = await fetch(LATEST_RELEASE, {
      headers: { "User-Agent": "learn-ops-cli", Accept: "application/vnd.github+json" },
    });
...
async function main() {
  ...
  // Pretty path for messages: "./learn-ops" for relative, as-is for absolute.
  ...
  console.log(`\n→ Cloning learn-ops${tag ? ` @ ${tag}` : ""} into ${display} ...`);
  ...
  console.log("\nlearn-ops is AI-agnostic — Claude Code, Codex, Qwen, OpenCode, Copilot, Antigravity and Grok all work.");
