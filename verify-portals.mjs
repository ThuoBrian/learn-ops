...
/**
 * verify-portals.mjs — ATS slug validator for learning-portals.yml.
 *
 * When a company is added to learning-portals.yml, its ATS slug (the path segment in
 ...
 *
 * Usage:
 *   node verify-portals.mjs                 # sweep tracked_companies in learning-portals.yml
...
const DEFAULT_PORTALS_PATH = process.env.LEARN_OPS_PORTALS || 'learning-portals.yml';
 ...
 *
 * @param {string} url - A `careers_url` or `api` value from learning-portals.yml.
 ...
 *
 * @param {string} filePath - Path to a learning-portals.yml.
 * @param {{fetchJson?: Function}} [deps]
...
async function main() {
  ...
  if (!found) {
    // Graceful no-op: fresh setups (and CI, which ships no learning-portals.yml) have
