 ...
 *
 * data/enrollments.md stays the source of truth. The SQLite DB is a derived
 ...
 *
 * Usage:
 *   node tracker.mjs sync [--check]             # (re)build enrollments.db from enrollments.md
 ...
 *   node tracker.mjs export [--out FILE]        # inverse: enrollments.db → canonical markdown (stdout by default)
 *   node tracker.mjs delete --num N [--dry-run] # remove one application row from enrollments.md + reindex
 *
 * query/history auto-resync when enrollments.md changed since the last sync,
...
const MD_PATH = process.env.LEARN_OPS_TRACKER || 'data/enrollments.md';
const DB_PATH = process.env.LEARN_OPS_TRACKER_DB
  || (MD_PATH.endsWith('.md') ? MD_PATH.slice(0, -3) + '.db' : MD_PATH + '.db');

// SQLite must never open the source of truth itself (an explicit
// LEARN_OPS_TRACKER_DB could point both names at the same file).
...
function loadStates() {
  if (!existsSync(STATES_PATH)) {
    console.error(`Error: ${STATES_PATH} not found — cannot validate statuses. Run from the learn-ops root.`);
    process.exit(1);
  }
...
// enrollments.md unless explicitly asked to via --out.
...
// sees a partially written enrollments.md (mirrors merge-tracker's writer).
...
// `delete --num N` removes one application row from enrollments.md and rebuilds
// the derived index. The markdown stays the source of truth: callers (incl. the
// web) orchestrate this script rather than editing enrollments.md directly, so
