 ...
 *   - id: string — matched against `provider:` in learning-portals.yml
 ...
 * `parser.command` + `parser.script` are set in learning-portals.yml.
 *
...
const PORTALS_PATH = process.env.LEARN_OPS_PORTALS || 'learning-portals.yml';
const PROFILE_PATH = process.env.LEARN_OPS_PROFILE || 'config/learner-profile.yml';
const SCAN_HISTORY_PATH = 'data/scan-history.tsv';
const PIPELINE_PATH = 'data/learning-inbox.md';
const APPLICATIONS_PATH = 'data/enrollments.md';
...
// ── Location filter ─────────────────────────────────────────────────
// Optional. If `location_filter` is absent from learning-portals.yml, all locations pass.
...
// Normalize a keyword list from learning-portals.yml: tolerates a bare string
...
// ── Content filter ──────────────────────────────────────────────────
// Optional. If `content_filter` is absent from learning-portals.yml, all jobs pass.
...
// ── Salary filter ───────────────────────────────────────────────────
// Optional. If `salary_filter` is absent from learning-portals.yml, all salaries pass.
// Semantics:
...
export function loadSeenUrls(policy = {}) {
  ...
  // learning-inbox.md — extract URLs from checkbox lines
  ...
  // enrollments.md — extract URLs from report links and any inline URLs
...
// in modes/learn-learning-inbox.md and expected by /learn-ops learn-pipeline.
const PIPELINE_SKELETON = `# Learning Inbox

Paste free course/cert URLs below as \`- [ ] {url}\` then run \`/learn-ops learn-pipeline\`.
...
// so existing learning-inbox.md files created before this change keep working.
  ...
  try {
    ...
    for (let i = 0; i < offers.length; i++) {
      ...
      if (result === 'expired') {
        ...
        // separately so they don't end up in learning-inbox.md but DO appear in scan-history
...
async function main() {
  ...
  // 2. Read learning-portals.yml
  if (!existsSync(PORTALS_PATH)) {
    console.error('Error: learning-portals.yml not found. Run onboarding first.');
    process.exit(1);
  }
  ...
  // them next scan, but never let them reach learning-inbox.md.
  ...
  // loadSeenUrls, but they never reach learning-inbox.md.
  ...
  if (agentHandoff.length > 0) {
    ...
    if (agentHandoff.length > 25) {
      console.log(`  … ${agentHandoff.length - 25} more omitted; narrow with --company or inspect learning-portals.yml`);
    }
  }
  ...
  console.log(`\n→ Run /learn-ops pipeline to evaluate new offers.`);
