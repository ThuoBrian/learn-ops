#!/usr/bin/env node
/**
 * merge-tracker.mjs — Merge batch tracker additions into enrollments.md
 *
 ...
 *
 * Run: node learn-ops/merge-tracker.mjs [--dry-run] [--verify]
 */
...
const LEARN_OPS = dirname(fileURLToPath(import.meta.url));
// Support both layouts: data/enrollments.md (boilerplate) and enrollments.md (original).
// LEARN_OPS_TRACKER overrides the path (used by tests and non-standard layouts).
const APPS_FILE_RAW = process.env.LEARN_OPS_TRACKER
  ? process.env.LEARN_OPS_TRACKER
  : existsSync(join(LEARN_OPS, 'data/enrollments.md'))
    ? join(LEARN_OPS, 'data/enrollments.md')
    : join(LEARN_OPS, 'enrollments.md');
const APPS_FILE = canonicalizeTrackerPath(APPS_FILE_RAW);
const TRACKER_DIR = dirname(APPS_FILE);
// LEARN_OPS_ADDITIONS overrides the additions dir (used by tests, mirrors LEARN_OPS_TRACKER).
const ADDITIONS_DIR = process.env.LEARN_OPS_ADDITIONS
  ? process.env.LEARN_OPS_ADDITIONS
  : join(LEARN_OPS, 'batch/tracker-additions');
const MERGED_DIR = join(ADDITIONS_DIR, 'merged');
const DRY_RUN = process.argv.includes('--dry-run');
const VERIFY = process.argv.includes('--verify');
const MIGRATE = process.argv.includes('--migrate');
const MERGE_HOLD_MS = Number(process.env.LEARN_OPS_MERGE_HOLD_MS) || 0;
const MERGE_READY_IPC = process.env.LEARN_OPS_MERGE_READY_IPC === '1';

const trackerLockKey = createHash('sha256').update(APPS_FILE).digest('hex').slice(0, 16);
const TRACKER_LOCK_DIR = resolveTrackerLockDir(process.env.LEARN_OPS_TRACKER_LOCK, trackerLockKey);

// The reports/ dir sits at the repo root, which is the tracker's parent in the
// data/ layout (data/enrollments.md) and the tracker's own dir at root layout.
 ...
 * generate. The tracker may live either at `data/enrollments.md` or at the
...
mkdirSync(join(LEARN_OPS, 'data'), { recursive: true });
 ...
 *
 * `LEARN_OPS_TRACKER_LOCK` exists for tests and unusual local layouts, but the
 ...
 * and use the learn-ops lock-name prefix. Invalid values are ignored and the
...
function resolveTrackerLockDir(envValue, lockKey) {
  ...
  const fallback = join(tmpRoot, `learn-ops-merge-tracker-${lockKey}.lock`);
  ...
  if (!basename(candidate).startsWith('learn-ops-merge-tracker-')) return fallback;
  return candidate;
}
 ...
 * - the regression test hook (`LEARN_OPS_MERGE_HOLD_MS`), which deliberately
 *   holds the first merge after it reads `enrollments.md` so a second merge can
 ...
 * filesystems and avoids exposing a partially written `enrollments.md` to other
...
try {
  trackerLock = await acquireTrackerLock(TRACKER_LOCK_DIR, {
    timeoutMs: Number(process.env.LEARN_OPS_TRACKER_LOCK_TIMEOUT_MS) || 60_000,
    retryMs: Number(process.env.LEARN_OPS_TRACKER_LOCK_RETRY_MS) || 75,
    staleMs: Number(process.env.LEARN_OPS_TRACKER_LOCK_STALE_MS) || 10 * 60_000,
  });
 ...
 * all of those variants here so enrollments.md keeps the states defined by
 ...
 *
 * @param {string} reportStr - Raw report cell from enrollments.md or TSV input.
...
// Column layout for the enrollments.md table. The tracker may use the original
...
// Neutralize characters that would corrupt the enrollments.md table. Both this
...
/**
 * Parse one Markdown enrollments.md table row into a tracker object.
 *
 ...
 *
 * @param {string} line - One line from enrollments.md.
...
if (!existsSync(APPS_FILE)) {
  console.log('No enrollments.md found. Nothing to merge into.');
  process.exit(0);
}
...
if (MIGRATE) {
  ...
  if (DRY_RUN) {
    ...
    console.log(`✅ Migration: rewrote ${changed} report link(s) in ${basename(APPS_FILE)} relative to ${TRACKER_DIR === LEARN_OPS ? 'repo root' : 'data/'}`);
  }
  process.exit(0);
}
...
for (const file of tsvFiles) {
  ...
  // so it resolves correctly when clicked from enrollments.md (see #760).
...
if (VERIFY && !DRY_RUN) {
  ...
  try {
    execFileSync('node', [join(LEARN_OPS, 'verify-pipeline.mjs')], { stdio: 'inherit' });
  } catch (e) {
    process.exit(1);
  }
}
