...
> "No active applications to follow up on. Apply to some roles first with `/learn-ops` and come back when they're aging."
...
- Suggest the user run `/learn-ops contacto {company}` to find the right person first
...
> - Trying a different contact via `/learn-ops contacto`
...
**To:** {email or "No contact found — run `/learn-ops contacto` first"}
