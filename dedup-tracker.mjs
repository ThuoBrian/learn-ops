#!/usr/bin/env node
/**
 * dedup-tracker.mjs — Remove duplicate entries from enrollments.md
 *
 ...
 *
 * Run: node learn-ops/dedup-tracker.mjs [--dry-run]
 */
...
// Support both layouts: data/enrollments.md (boilerplate) and enrollments.md
// (original). LEARN_OPS_TRACKER lets tests point the script at an isolated
// fixture so the real user tracker is never touched.
const APPS_FILE = process.env.LEARN_OPS_TRACKER
 ...
 *
 * @param {string} name - Company name from an enrollments.md row.
 ...
 *
 * @param {string} status - Raw status cell from enrollments.md.
 ...
 *
 * @param {string} reportStr - Raw report cell from enrollments.md.
 ...
 *
 * @param {object} a - First parsed enrollments.md row.
 * @param {object} b - Second parsed enrollments.md row.
...
/**
 * Parse one Markdown table row from enrollments.md into a tracker object.
 *
...
if (!existsSync(APPS_FILE)) {
  console.log('No enrollments.md found. Nothing to dedup.');
  process.exit(0);
}
...
if (!DRY_RUN && removed > 0) {
  ...
  console.log('✅ Written to enrollments.md (backup: enrollments.md.bak)');
} else if (DRY_RUN) {
