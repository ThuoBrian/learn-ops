#!/usr/bin/env node
/**
 * verify-pipeline.mjs — Health check for learn-ops pipeline integrity
 *
 * Checks:
 ...
 *
 * Run: node learn-ops/verify-pipeline.mjs
 */
...
const ADDITIONS_DIR = join(LEARN_OPS, 'batch/tracker-additions');
const REPORTS_DIR = join(LEARN_OPS, 'reports');
const STATES_FILE = existsSync(join(LEARN_OPS, 'templates/states.yml'))
  ? join(LEARN_OPS, 'templates/states.yml')
  : join(LEARN_OPS, 'states.yml');
...
if (!existsSync(APPS_FILE)) {
  console.log('\n📊 No enrollments.md found. This is normal for a fresh setup.');
...
console.log(`\n📊 Checking ${entries.length} entries in enrollments.md\n`);
...
for (const e of entries) {
  ...
  if (!match) continue;
  const link = match[1];
  if (!existsSync(join(TRACKER_DIR, link)) && !existsSync(join(LEARN_OPS, link))) {
