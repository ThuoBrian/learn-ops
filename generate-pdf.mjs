 ...
 *
 * Usage:
 *   node learn-ops/generate-pdf.mjs <input.html> <output.pdf> [--format=letter|a4] [--report=NNN]
 *
 ...
 * learn-ops root with forward slashes. One row per PDF path; when a report
...
export async function renderHtmlToPdf(html, outputPath, opts = {}) {
  ...
  const tmpHtmlPath = resolve(baseDir, `.learn-ops-render-${randomUUID()}.html`);
