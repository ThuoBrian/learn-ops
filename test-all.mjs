...
/**
 * test-all.mjs — Comprehensive test suite for learn-ops
 *
 ...
 * any freshly cloned learn-ops checkout with only Node.js available.
 *
 ...
 *
 * @param {string} path - Path relative to the learn-ops repository root.
...
const scripts = [
  ...
  // data/enrollments.md (or data/learning-inbox.md) in place. On a provisioned working
  ...
  // tests inside their active learn-ops workspace.
  ...
  // default learning-portals.yml because end-user workspaces often have a real user-layer
  // portals file that would trigger a live remote sweep during tests.
  { name: 'verify-portals.mjs --file .tmp-test-missing-learning-portals.yml', expectExit: 0 },
...
const systemFiles = [
  ...
  '.claude/skills/learn-ops/SKILL.md',
  '.opencode/skills/learn-ops/SKILL.md',
  '.qwen/skills/learn-ops/SKILL.md',
  '.antigravitycli/skills/learn-ops/SKILL.md',
  '.grok/skills/learn-ops/SKILL.md',
];
...
const userFiles = [
  'config/learner-profile.yml', 'modes/_profile.md', 'learning-portals.yml',
];
...
const expectedModes = [
  ...
  'batch.md', 'apply.md', 'auto-learning-inbox.md', 'contacto.md', 'deep.md',
  'ofertas.md', 'learning-inbox.md', 'project.md', 'tracker.md', 'training.md',
...
for (const skillPath of ['.claude/skills/learn-ops/SKILL.md', '.agents/skills/learn-ops/SKILL.md']) {
  if (!fileExists(skillPath)) {
    fail(`${skillPath} is missing`);
    continue;
  }
  const skill = readFile(skillPath);
  if (skill.includes('/learn-ops latex')) {
    pass(`${skillPath} exposes /learn-ops latex in discovery menu`);
  } else {
    fail(`${skillPath} does not expose /learn-ops latex in discovery menu`);
  }
}
...
const autoPipelineMode = readFile('modes/auto-learning-inbox.md');
if (
...
const pipelineMode = readFile('modes/learning-inbox.md');
if (
...
// learning-inbox.md location column (B1): formatPipelineOffer appends location as a
...
try {
  ...
  // learning-inbox.md compensation column (B3): formatCompensation renders the parsed
...
try {
  ...
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'learn-ops-missing-pipeline-'));
  ...
  try {
    ...
    const pipeline = readFileSync(join(fixtureRoot, 'data', 'learning-inbox.md'), 'utf-8');
    if (
      pipeline.includes('# Pipeline') &&
      pipeline.includes('## Pending') &&
      pipeline.includes('- [ ] https://jobs.example.com/1 | Acme | Engineer')
    ) {
      pass('scan.mjs creates data/learning-inbox.md before appending offers on fresh installs (#1252)');
    } else {
...
try {
  const tmp = mkdtempSync(join(tmpdir(), 'learn-ops-portals-validator-'));
...
const canonicalSkill = '.agents/skills/learn-ops/SKILL.md';
const symlinks = [
  '.claude/skills/learn-ops/SKILL.md',
  '.opencode/skills/learn-ops/SKILL.md',
  '.qwen/skills/learn-ops/SKILL.md',
...
if (
  ...
  /\/learn-ops/.test(canonicalContent ?? '')
) {
  pass('learn-ops skill router documents the Codex invocation model');
} else {
  fail('learn-ops skill router is missing Codex invocation guidance');
}
...
if (
  ...
  /(slash commands?.*not guaranteed|prompt|\/learn-ops.*unavailable)/i.test(agentsDoc)
) {
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'learn-ops-skills-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'learn-ops');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'learn-ops');
    const opencodeDir = join(fixtureRoot, '.opencode', 'skills', 'learn-ops');
    mkdirSync(canonicalDir, { recursive: true });
    mkdirSync(claudeDir, { recursive: true });
    mkdirSync(opencodeDir, { recursive: true });

    const fixtureSkill = '---\nname: learn-ops\n---\n\n# canonical skill\n';
    const pointer = '../../../.agents/skills/learn-ops/SKILL.md';
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'learn-ops-ensure-skills-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'learn-ops');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'learn-ops');
    mkdirSync(canonicalDir, { recursive: true });
    ...
    const grokSkill = readFileSync(join(fixtureRoot, '.grok', 'skills', 'learn-ops', 'SKILL.md'), 'utf-8');
    const claudeSkill = readFileSync(join(claudeDir, 'SKILL.md'), 'utf-8');
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'learn-ops-skills-unreadable-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'learn-ops');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'learn-ops');
    mkdirSync(canonicalDir, { recursive: true });
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'learn-ops-skills-entry-dir-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'learn-ops');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'learn-ops');
    const opencodeDir = join(fixtureRoot, '.opencode', 'skills', 'learn-ops');
    ...
    const materialized = updater.materializeSkillEntrypoints(fixtureRoot);
    const opencodeSkill = readFileSync(join(opencodeDir, 'SKILL.md'), 'utf-8');
    if (JSON.stringify(materialized) === JSON.stringify(['.opencode/skills/learn-ops/SKILL.md']) && opencodeSkill === fixtureSkill) {
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'learn-ops-skill-git-'));
  ...
  try {
    ...
    writeFileSync(join(claudeDir, 'SKILL.md'), pointer);
    writeFileSync(join(opencodeDir, 'SKILL.md'), pointer);
    gitRun(['add', '--', '.agents/skills/learn-ops/SKILL.md']);

    const pointerBlob = gitRun(['hash-object', '-w', '--stdin'], { input: pointer });
    gitRun(['update-index', '--add', '--cacheinfo', `120000,${pointerBlob},.claude/skills/learn-ops/SKILL.md`]);
    gitRun(['update-index', '--add', '--cacheinfo', `120000,${pointerBlob},.opencode/skills/learn-ops/SKILL.md`]);
    ...
    const claudeIndex = gitRun(['ls-files', '-s', '--', '.claude/skills/learn-ops/SKILL.md']);
    const opencodeIndex = gitRun(['ls-files', '-s', '--', '.opencode/skills/learn-ops/SKILL.md']);
    ...
    const claudeBlob = gitRaw(['show', ':.claude/skills/learn-ops/SKILL.md']);
    const opencodeBlob = gitRaw(['show', ':.opencode/skills/learn-ops/SKILL.md']);
...
try {
  ...
  if (pipelineProcessed === '[12](../reports/012-acme-2026-01-04.md)') {
    pass('pipeline processed links are relative to data/learning-inbox.md (#1126)');
  } else {
  ...
  try {
    ...
    run(NODE, ['merge-tracker.mjs', '--migrate'], { env: { ...process.env, LEARN_OPS_TRACKER: tracker } });
  ...
  const followupTmp = mkdtempSync(join(tmpdir(), 'learn-ops-followup-link-'));
  try {
    ...
    const appsFile = join(followupTmp, 'data', 'enrollments.md');
...
// and could delete an already-Applied row because data/enrollments.md is
...
try {
  ...
  const dedupTmp = mkdtempSync(join(tmpdir(), 'learn-ops-dedup-'));
  try {
    mkdirSync(join(dedupTmp, 'data'));
    const tracker = join(dedupTmp, 'data', 'enrollments.md');
    writeFileSync(tracker,
      '# Applications Tracker\n\n' +
      '| # | Date | Company | Role | Score | Status | PDF | Report | Notes |\n' +
      '|---|------|---------|------|-------|--------|-----|--------|-------|\n' +
    ...
    const dedupResult = run(NODE, ['dedup-tracker.mjs'], { env: { ...process.env, LEARN_OPS_TRACKER: tracker } });
    if (dedupResult === null) {
...
try {
  const rebuildTmp = mkdtempSync(join(tmpdir(), 'learn-ops-rebuild-'));
  try {
    mkdirSync(join(rebuildTmp, 'data'));
    const tracker = join(rebuildTmp, 'data', 'enrollments.md');
    ...
    const r = run(NODE, ['dedup-tracker.mjs'], { env: { ...process.env, LEARN_OPS_TRACKER: tracker } });
    if (r === null) {
...
try {
  const locTmp = mkdtempSync(join(tmpdir(), 'learn-ops-dedup-loc-'));
  try {
    mkdirSync(join(locTmp, 'data'));
    const tracker = join(locTmp, 'data', 'enrollments.md');
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'learn-ops-merge-'));
  try {
    ...
    const tracker = join(mergeTmp, 'data', 'enrollments.md');
    writeFileSync(tracker,
      '# Applications Tracker\n\n' +
      '| # | Date | Company | Role | Score | Status | PDF | Report | Notes |\n' +
      '|---|------|---------|------|-------|--------|-----|--------|-------|\n' +
    ...
    const mergeResult = run(NODE, ['merge-tracker.mjs'], { env: { ...process.env, LEARN_OPS_TRACKER: tracker, LEARN_OPS_ADDITIONS: additionsDir } });
    if (mergeResult === null) {
...
try {
  const col912Tmp = mkdtempSync(join(tmpdir(), 'learn-ops-merge-912-'));
  try {
    ...
    const col912Tracker = join(col912Tmp, 'data', 'enrollments.md');
    ...
    const col912Result = run(NODE, ['merge-tracker.mjs'], {
      env: { ...process.env, LEARN_OPS_TRACKER: col912Tracker, LEARN_OPS_ADDITIONS: col912Additions },
    });
...
// old enrollments.md snapshot and then write back independently, one process
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'learn-ops-merge-lock-'));
  /**
  ...
  function spawnMerge(additionsDir, holdMs = 0) {
    ...
    const result = new Promise(resolve => {
      const child = spawn(NODE, ['merge-tracker.mjs'], {
        ...
        env: {
          ...process.env,
          LEARN_OPS_TRACKER: join(mergeTmp, 'data', 'enrollments.md'),
          LEARN_OPS_ADDITIONS: additionsDir,
          LEARN_OPS_TRACKER_LOCK: join(mergeTmp, 'learn-ops-merge-tracker-fixture.lock'),
          LEARN_OPS_MERGE_HOLD_MS: String(holdMs),
          LEARN_OPS_MERGE_READY_IPC: '1',
        },
  ...
  try {
    ...
    writeFileSync(join(mergeTmp, 'data', 'enrollments.md'),
      '# Applications Tracker\n\n' +
      '| # | Date | Company | Role | Score | Status | PDF | Report | Notes |\n' +
      '|---|------|---------|------|-------|--------|-----|--------|-------|\n');
    ...
    const merged = readFileSync(join(mergeTmp, 'data', 'enrollments.md'), 'utf-8');
...
try {
  ...
  for (const f of ['cv.md', 'config/learner-profile.yml', 'modes/_profile.md', 'learning-portals.yml']) {
...
// enrollments.md is the source of truth; enrollments.db is a derived index
...
if (!sqliteAvailable) {
  ...
  try {
    const idxTmp = mkdtempSync(join(tmpdir(), 'learn-ops-index-'));
    try {
      const md = join(idxTmp, 'enrollments.md');
      const env = { ...process.env, LEARN_OPS_TRACKER: md };
      ...
      if (trackerRun(['sync']) === null) {
        ...
        if (readFileSync(md, 'utf-8') === clean) {
          ...
          fail('sync/export modified enrollments.md (source of truth violated)');
        }
      }
      ...
      if (fresh.length === 1) {
        pass('query auto-resyncs when enrollments.md changed since last sync');
      } else {
...
try {
  ...
  const hit = sj.detect({ name: 'SJ', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=learn-ops' });
  ...
  const fakeJobs = {
    jobs: [
      { title: 'Senior Dev', url: 'https://solid.jobs/o/abc123/learn-ops', company: 'Acme Corp', locations: ['Warszawa', 'Remote'] },
      { title: 'Junior Dev', url: 'https://solid.jobs/o/def456/learn-ops', company: 'Beta Inc', locations: ['Kraków'] },
    ],
  };
  const parsed = await sj.fetch(
    { name: 'SolidJobs IT', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=learn-ops' },
  ...
  if (parsed[0].title === 'Senior Dev' && parsed[0].url === 'https://solid.jobs/o/abc123/learn-ops') {
  ...
  const noCompanyJobs = { jobs: [{ title: 'Tester', url: 'https://solid.jobs/o/xyz/learn-ops', locations: [] }] };
  ...
  const mixedJobs = {
    jobs: [
      { title: 'Has URL', url: 'https://solid.jobs/o/1/learn-ops', company: 'A', locations: [] },
  ...
  const stringLocJobs = { jobs: [{ title: 'Dev', url: 'https://solid.jobs/o/2/learn-ops', company: 'X', locations: 'Warsaw' }] };
  ...
  const malformedMembers = { jobs: [null, 7, { title: 'OK', url: 'https://solid.jobs/o/3/learn-ops', company: 'Z' }] };
  const safeParsed = await sj.fetch(
    { name: 'SolidJobs IT', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=learn-ops' },
    { transport: 'http', fetchJson: async () => malformedMembers, fetchText: async () => '' },
  );
  if (safeParsed.length === 1 && safeParsed[0].url === 'https://solid.jobs/o/3/learn-ops') {
...
try {
  ...
  // Release Please tags carry the component prefix (learn-ops-v1.9.0); the
  // prefix must be stripped or the releases-API fallback is dead code (#923).
  if (parse('learn-ops-v1.9.0') === '1.9.0') {
    pass('SEMVER_RE parses Release Please component-prefixed tag (learn-ops-v1.9.0 → 1.9.0)');
  } else {
    fail(`SEMVER_RE failed on learn-ops-v1.9.0 (got ${parse('learn-ops-v1.9.0')}) — releases-API fallback is dead code (#923)`);
  }
  ...
  if (parse('learn-ops') === null && parse('v1.9') === null) {
    pass('SEMVER_RE rejects non-semver input');
  } else {
    fail(`SEMVER_RE matched non-semver input (learn-ops → ${parse('learn-ops')}, v1.9 → ${parse('v1.9')})`);
  }
} catch (e) {
...
try {
  ...
  // only when config/learner-profile.yml sets candidate.photo; otherwise it stays empty.
