...
/**
 * update-system.mjs — Safe auto-updater for learn-ops
 *
...
const CANONICAL_REPO = 'https://github.com/santifer/learn-ops.git';
const RAW_VERSION_URL = 'https://raw.githubusercontent.com/santifer/learn-ops/main/VERSION';
const RELEASES_API = 'https://api.github.com/repos/santifer/learn-ops/releases/latest';

// Matches a semver, with or without a leading `v` and an optional
// Release Please component prefix (e.g. `learn-ops-v1.9.0` → `1.9.0`).
...
const SYSTEM_PATHS = [
  ...
  'fonts/',
  'examples/',
  'config/learner-profile.example.yml',
  'config/archetypes.yml',
  'learning-portals.yml',
  // Docs + infra
  '.env.example',
  '.agents/',
  '.claude/skills/',
...
async function check() {
  ...
  const [rawVersion, releaseRaw] = await Promise.all([
    ...
    curlGet(RELEASES_API, [
      ...
      '--header', 'User-Agent: learn-ops-update-checker',
    ]),
  ]);
...
async function apply() {
  ...
  const isReexec = process.env.LEARN_OPS_UPDATE_REEXEC === '1';
  ...
  try {
    ...
    const backupBranch = process.env.LEARN_OPS_UPDATE_BACKUP_BRANCH || updateBackupBranchName(local);
    if (!isReexec) {
      try {
    ...
    if (!isReexec) {
      try {
        ...
        execFileSync(process.execPath, ['update-system.mjs', 'apply'], {
          ...
          env: {
            ...process.env,
            LEARN_OPS_UPDATE_REEXEC: '1',
            LEARN_OPS_UPDATE_BACKUP_BRANCH: backupBranch,
          },
        });
        return;
      } catch (err) {
