#!/usr/bin/env node
/**
 * reconcile-pipeline.mjs — Sync learning-inbox.md "Pendientes" with batch-state.tsv
 *
 * THE PROBLEM
 * batch-runner.sh records every evaluated offer in batch/batch-state.tsv, but
 * it never writes back to data/learning-inbox.md. Offers processed via batch mode
 * therefore stay in the "Pendientes" section forever — the next scan and the
 * next `/learn-ops pipeline` run both re-surface them, and they get evaluated
 * again (duplicate reports, duplicate tracker rows).
 *
 * WHAT THIS DOES
 * For each `completed` / `skipped` entry in batch-state.tsv whose URL is still
 * sitting in learning-inbox.md "Pendientes", move that line to "Procesadas" with its
...
if (process.argv.includes('-h') || process.argv.includes('--help')) {
  ...
  console.log('  Moves batch-processed offers out of learning-inbox.md "Pendientes" into "Procesadas".');
  process.exit(0);
}
...
function resolveInsideRepo(inputPath, fallbackPath, flag) {
  ...
  try {
    repoReal = realpathSync(LEARN_OPS);
...
const defaultPipeline = existsSync(join(LEARN_OPS, 'data/learning-inbox.md'))
  ? join(LEARN_OPS, 'data/learning-inbox.md')
  : join(LEARN_OPS, 'learning-inbox.md');
const PIPELINE_FILE = resolveInsideRepo(argValue('--pipeline'), defaultPipeline, '--pipeline');
const STATE_FILE = resolveInsideRepo(argValue('--state'), join(LEARN_OPS, 'batch/batch-state.tsv'), '--state');
const REPORTS_DIR = join(LEARN_OPS, 'reports');
...
if (!existsSync(PIPELINE_FILE)) {
  console.log('No learning-inbox.md found — nothing to reconcile.');
  process.exit(0);
}
...
if (pendStart < 0) {
  console.log('No "Pendientes" section in learning-inbox.md — nothing to reconcile.');
  process.exit(0);
}
...
for (let i = pendStart + 1; i < pendEnd; i++) {
  ...
  const reportLink = normalizeReportLink(`[${num}](reports/${reportFile})`, dirname(PIPELINE_FILE), LEARN_OPS);
...
console.log('=== Reconcile learning-inbox.md ===');
...
if (removeIdx.size === 0) {
  console.log('✅ learning-inbox.md already in sync — nothing to reconcile.');
  process.exit(0);
}
...
console.log(`✅ learning-inbox.md updated (backup: ${PIPELINE_FILE}.pre-reconcile.bak)`);
